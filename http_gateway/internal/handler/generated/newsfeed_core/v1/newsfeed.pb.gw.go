...
/*
Package newsfeedv1 is a reverse proxy.
...
func RegisterNewsfeedServiceHandlerServer(ctx context.Context, mux *runtime.ServeMux, server NewsfeedServiceServer) error {
	mux.Handle(http.MethodGet, pattern_NewsfeedService_GetNewsfeed_0, func(w http.ResponseWriter, req *http.Request, pathParams map[string]string) {
		...
		resp, md, err := local_request_NewsfeedService_GetNewsfeed_0(annotatedContext, inboundMarshaler, server, req, pathParams)
...
// RegisterNewsfeedServiceHandlerFromEndpoint is same as RegisterNewsfeedServiceHandler but
// automatically dials to "endpoint" and closes the connection when "ctx" gets done.
func RegisterNewsfeedServiceHandlerFromEndpoint(ctx context.Context, mux *runtime.ServeMux, endpoint string, opts []grpc.DialOption) (err error) {
...
// RegisterNewsfeedServiceHandler registers the http handlers for service NewsfeedService to "mux".
// The handlers forward requests to the grpc endpoint over "conn".
func RegisterNewsfeedServiceHandler(ctx context.Context, mux *runtime.ServeMux, conn *grpc.ClientConn) error {
	return RegisterNewsfeedServiceHandlerClient(ctx, mux, NewNewsfeedServiceClient(conn))
}

// RegisterNewsfeedServiceHandlerClient registers the http handlers for service NewsfeedService
...
func RegisterNewsfeedServiceHandlerClient(ctx context.Context, mux *runtime.ServeMux, client NewsfeedServiceClient) error {
	mux.Handle(http.MethodGet, pattern_NewsfeedService_GetNewsfeed_0, func(w http.ResponseWriter, req *http.Request, pathParams map[string]string) {
		...
		resp, md, err := request_NewsfeedService_GetNewsfeed_0(annotatedContext, inboundMarshaler, client, req, pathParams)
		annotatedContext = runtime.NewServerMetadataContext(annotatedContext, md)
		if err != nil {
			runtime.HTTPError(annotatedContext, mux, outboundMarshaler, w, req, err)
			return
...
var (
	pattern_NewsfeedService_GetNewsfeed_0 = runtime.MustPattern(runtime.NewPattern(1, []int{2, 0, 2, 1, 2, 2}, []string{"api", "v1", "newsfeed"}, ""))
)

var (
	forward_NewsfeedService_GetNewsfeed_0 = runtime.ForwardResponseMessage
)
